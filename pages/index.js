...
const CSS = `
  * { box-sizing: border-box; }
  @media (max-width: 768px) {
    ...
    .blindaje-grid    { grid-template-columns: 1fr 1fr !important; }
  ...
  @media (max-width: 480px) {
    ...
    .blindaje-grid    { grid-template-columns: 1fr !important; }
  ...
  return (
    <>
      <Head>
        ...
        <link rel="canonical" href="https://www.emporioinmobiliario.com.mx/" />
      </Head>
...
{/* HOT WEEK */}
<div style={{
background:"linear-gradient(90deg,#C8102E 0%,#9F1239 100%)",
padding:"14px 20px",
position:"relative",
overflow:"hidden"
}}>
...
Beneficios especiales por tiempo limitado
</h3>
...
📺 Aparta una propiedad participante y recibe beneficios exclusivos al cerrar tu operación*
</p>
...
*Aplican términos y condiciones
        ...
        {/* BLINDAJE LEGAL */}
        <div className="sec-pad" style={{ padding: "72px 32px", background: "#1a1a2e", position: "relative", overflow: "hidden" }}>
          <div style={{ position: "absolute", top: -80, right: -80, width: 400, height: 400, borderRadius: "50%", background: "radial-gradient(circle, rgba(200,16,46,0.15) 0%, transparent 70%)", pointerEvents: "none" }} />
          <div style={{ maxWidth: 1200, margin: "0 auto", position: "relative", zIndex: 1 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 40, flexWrap: "wrap", gap: 16 }}>
              <div>
                <p style={{ fontSize: 11, color: "#C8102E", fontWeight: 700, letterSpacing: "0.2em", textTransform: "uppercase", margin: "0 0 10px" }}>Nuevo servicio</p>
                <h2 className="sec-title" style={{ fontSize: 36, fontWeight: 900, color: "#fff", margin: "0 0 8px" }}>Emporio <span style={{ color: "#C8102E" }}>Blindaje Legal</span></h2>
                <p style={{ fontSize: 15, color: "rgba(255,255,255,0.55)", margin: 0, maxWidth: 480, lineHeight: 1.6 }}>Renta tu inmueble con protección jurídica total. Investigamos al candidato, elaboramos el contrato y te respaldamos si algo falla.</p>
              </div>
              <a href="/blindaje-legal" style={{ display: "inline-flex", alignItems: "center", gap: 6, color: "#fca5a5", fontWeight: 700, fontSize: 14, textDecoration: "none", whiteSpace: "nowrap" }}>Ver todo el servicio →</a>
            </div>
            <div className="blindaje-grid" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16, marginBottom: 32 }}>
              {[
                { icon: "🔍", title: "Investigación completa",    desc: "Ingresos, referencias personales, laborales y familiares, antecedentes jurídicos (Buró México) y validación de documentos." },
                { icon: "📄", title: "Contrato blindado",         desc: "Elaborado por especialistas. Jurídicamente sólido desde la primera cláusula." },
                { icon: "🛡️", title: "Póliza jurídica formal",   desc: "Cobertura activa durante los 12 meses de vigencia del arrendamiento." },
                { icon: "⚖️", title: "Recuperación del inmueble", desc: "Si el inquilino incumple, activamos el proceso extrajudicial y judicial con nuestro despacho aliado." },
              ].map((item, i) => (
                <div key={i} style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 14, padding: "24px 20px" }}>
                  <div style={{ fontSize: 28, marginBottom: 12 }}>{item.icon}</div>
                  <h4 style={{ fontSize: 14, fontWeight: 700, color: "#fff", margin: "0 0 6px" }}>{item.title}</h4>
                  <p style={{ fontSize: 12, color: "rgba(255,255,255,0.45)", lineHeight: 1.6, margin: 0 }}>{item.desc}</p>
                </div>
              ))}
            </div>
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
              <a href="/blindaje-legal" style={{ background: "#C8102E", color: "#fff", padding: "13px 24px", borderRadius: 10, fontWeight: 800, fontSize: 14, textDecoration: "none" }}>🛡️ Conocer Emporio Blindaje Legal</a>
              <a href="https://app.emporioinmobiliario.com.mx/registro-propietario" target="_blank" rel="noreferrer" style={{ background: "rgba(255,255,255,0.08)", color: "rgba(255,255,255,0.8)", padding: "13px 24px", borderRadius: 10, fontWeight: 700, fontSize: 14, textDecoration: "none", border: "1px solid rgba(255,255,255,0.15)" }}>🏠 Registrar mi propiedad →</a>
              <a href="https://app.emporioinmobiliario.com.mx/solicitud-inquilino" target="_blank" rel="noreferrer" style={{ background: "rgba(255,255,255,0.08)", color: "rgba(255,255,255,0.8)", padding: "13px 24px", borderRadius: 10, fontWeight: 700, fontSize: 14, textDecoration: "none", border: "1px solid rgba(255,255,255,0.15)" }}>🔑 Soy inquilino →</a>
            </div>
          </div>
        </div>
