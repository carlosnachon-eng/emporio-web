...
function cleanMarketingValue(value, max) {
  const normalized = clean(value, max);
  if (!normalized || /@/.test(normalized)) return null;
  const phoneCandidate = normalized.replace(/[\s()+.\-]/g, "");
  if (/^\d{7,15}$/.test(phoneCandidate)) return null;
  return normalized;
}
...
export function deriveWhatsappTouch({ url, referrer, siteOrigin, now = new Date().toISOString() }) {
  ...
  const utmSource = cleanMarketingValue(parsed.searchParams.get("utm_source"), MAX.source);
  const utmMedium = cleanMarketingValue(parsed.searchParams.get("utm_medium"), MAX.medium);
  return Object.freeze({
    source: utmSource || referral?.source || "direct",
    medium: utmMedium || (utmSource ? "campaign" : referral?.medium || "(none)"),
    campaign: cleanMarketingValue(parsed.searchParams.get("utm_campaign"), MAX.campaign),
    content: cleanMarketingValue(parsed.searchParams.get("utm_content"), MAX.content),
    term: cleanMarketingValue(parsed.searchParams.get("utm_term"), MAX.term),
...
export function validateWhatsappTouch(value) {
  ...
  return Object.freeze({
    source: cleanMarketingValue(value.source, MAX.source) || "direct",
    medium: cleanMarketingValue(value.medium, MAX.medium) || "(none)",
    campaign: cleanMarketingValue(value.campaign, MAX.campaign),
    content: cleanMarketingValue(value.content, MAX.content),
    term: cleanMarketingValue(value.term, MAX.term),
    landing_path: path,
