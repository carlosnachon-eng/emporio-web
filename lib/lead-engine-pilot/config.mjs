...
export function loadLeadEngineConfig(env) {
  ...
  if (!new Set(["preview", "development"]).has(deploymentEnvironment)) {
    throw new LeadEngineConfigurationError(
      LEAD_ENGINE_ERROR.unsafeTarget,
      "El piloto sólo puede activarse en Preview o Development.",
    );
  }
  ...
  if (allowed !== projectRef || blocked.has(projectRef)) {
    throw new LeadEngineConfigurationError(
      LEAD_ENGINE_ERROR.unsafeTarget,
      "El Project Reference activo no coincide exclusivamente con la allowlist Demo.",
    );
  }
  ...
  if (claims && (claims.role !== "service_role" || claims.ref !== projectRef)) {
    throw new LeadEngineConfigurationError(
      LEAD_ENGINE_ERROR.unsafeTarget,
      "La service role JWT no pertenece al proyecto Demo permitido.",
    );
  }
