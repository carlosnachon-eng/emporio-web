...
import { validateAttributionPayload } from "./attribution.mjs";
...
export async function capturePropertyLeadPilot({
  ...
  return runLeadEngineFailOpen({
    ...
    execute: async () => {
      ...
      const attribution = validateAttributionPayload(input?.attribution);
      ...
      const { data, error } = await prepared.client.rpc(
        "lead_engine_capture_property_pilot",
        {
          ...
          p_attribution_id: attribution?.attributionId || null,
          p_first_touch: attribution?.firstTouch || null,
          p_last_touch: attribution?.lastTouch || null,
        },
      );
      if (error) {
